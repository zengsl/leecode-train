...
//                System.out.println("ans:" + ans);
    ...
    public static int prime() {
        /*pointCnt = 1;
        HEAP[heapSize++] = new int[]{TO[HEAD[1]], 0};
        heapInsert(heapSize - 1);
        WHERE[HEAD[1]] = 0;*/
        pointCnt = 1;
        WHERE[1] = WHERE_USED;
        for (int nextEdge = HEAD[1]; nextEdge != 0; nextEdge = NEXT[nextEdge]) {
            addOrUpdate(nextEdge);
        }
...
//            System.out.println(Arrays.toString(e));
            for (int nextEdge = HEAD[e[0]]; nextEdge != 0; nextEdge = NEXT[nextEdge]) {
                addOrUpdate(nextEdge);
            }
        }
        return ans;
    ...
    private static void addOrUpdate(int nextEdge) {
        int nextTo = TO[nextEdge];
