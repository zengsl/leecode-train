...
//given a 0-indexed integer array time where time[i] denotes how many months it takes 
//to complete the (i+1)ᵗʰ course. 
//
...
// 
//
// Related Topics 图 拓扑排序 数组 动态规划 👍 132 👎 0
 ...
 */
public class ParallelCoursesIii2 {
    public static void main(String[] args) {
        Solution solution = new ParallelCoursesIii2().new Solution();
//        System.out.println(solution.minimumTime(3, new int[][]{{1, 3}, {2, 3}}, new int[]{3, 2, 5}));
        System.out.println(solution.minimumTime(1, new int[][]{}, new int[]{1}));
    }
    ...
    class Solution {
        ...
        public int minimumTime(int n, int[][] relations, int[] time) {
            ...
            for (int[] relation : relations) {
                ADJACENCY_TABLE.get(relation[0]).add(relation[1]);
                INDEGREE[relation[1]]++;
            }
            ...
            while (l < r) {
                int f = DEQUE[l++];
                cost[f] += time[f - 1];
                costs = Math.max(costs, cost[f]);
                for (int to : ADJACENCY_TABLE.get(f)) {
                    cost[to] = Math.max(cost[to], cost[f]);
                    if (--INDEGREE[to] == 0) {
                        DEQUE[r++] = to;
                    }
                }
